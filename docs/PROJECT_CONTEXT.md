...
最后核对日期：2026-09-21
代码分支：`main`；M1 至 M8 已完成，最终 AI 函数 v35 已部署并完成 24 题质量与性能验收。
...
M1 至 M8 已全部完成。后续优先级转为日常运营验证：在资料到期前复核公交、食堂等动态知识；补浏览器端自动化冒烟测试；核对隐私政策、模型供应商披露和线上安全设置。M8 的完整结果见 [AI_M8_FINAL_REPORT.md](AI_M8_FINAL_REPORT.md)。
...
M8 完成记录：首次 v34 半程复测发现通用时间/条件加权会把转专业资料混入无关主题；v35 增加主题匹配门槛，年度通知、政策和历史参考只在主题一致时参与角色加权。最终 24/24 完整通过，关键点、引用、历史标注和无依据克制均为 100%；首段文字中位数 2.073 秒、P95 3.888 秒，完整回答中位数 7.458 秒、P95 11.441 秒，流式中断为 0。v35 bundle SHA-256 为 `bc7d8773954e6f2f3eeb8edbe4df2327447d14bed5e3ffe81763bc62164aef8b`，v34 回退包保存在 `out/ai-quality/rollback-pre-m8-v34/`。
